...
def RefreshRoomState(entities=None):
    ...
    parent = get_tree().find(".//*[@resource-id='3d-map-out-div-9527']")
    if parent is None:
        print("⚠️ No map found for RefreshRoomState()")
        return [] if entities is None else entities

    buttons = parent.findall(".//*[@class='android.widget.Button']")
    button_states = []
    for btn in buttons:
        raw_text = btn.get("text", "") or ""
        android_name = raw_text[2:].strip() if len(raw_text) > 2 else raw_text.strip()
        if not android_name:
            continue
        enabled = bool(int(btn.get("index", "0")))
        button_states.append((android_name, enabled))
    ...
    if entities is None:
        return [
            ...
            for name, enabled in button_states
        ]
...
def enbl_room(room_name):
    navigate_to("Robot")
    tree = get_tree()
    target = find_by_text(tree, room_name, contains=True)
